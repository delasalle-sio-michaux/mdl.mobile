...
class DAO
{
	...
	{	
		$lesReservations =  $this->listeReservations($nomUser);
		foreach($lesReservations as $uneReservation){
			if($uneReservation->getId()==$idReservation)
				return true;
		}
		return false;			
	}
	...
	{	
		// préparation de la requete de recherche
		$txt_req = "Select mrbs_entry.id as id, timestamp, start_time, end_time, room_name, status, digicode";
		$txt_req = $txt_req . " from mrbs_entry, mrbs_room, mrbs_entry_digicode";
		$txt_req = $txt_req . " where mrbs_entry.room_id = mrbs_room.id";
		$txt_req = $txt_req . " and mrbs_entry.id = mrbs_entry_digicode.id";
		$txt_req = $txt_req . " and mrbs_entry.id = :idRes";
		$txt_req = $txt_req . " order by start_time, room_name";
	...
	{	// préparation de la requete de recherche
		...
		{	// création d'un objet Reservation
			...
			$unUser = new Utilisateur($unId, $unLevel, $unNom, $unPassword, $unEmail);
			
			$listeReservations = $this->listeReservations($nomUser);
			
			foreach($listeReservations as $uneReservation ){
			 	$unUser->ajouteReservation($uneReservation);
			}
			// ajout de la réservation à la collection
			return $unUser;
		}
	...
	// modifié par Enguerrand le 22/9/2015
	...
	{	
		...
		$txt_req = "UPDATE mrbs_users SET password = :newpass WHERE name=:unName";
		$req = $this->cnx->prepare($txt_req);
		$req->bindValue("newpass", $nouveauMdp, PDO::PARAM_STR);
		$req->bindValue("unName", $nomUser, PDO::PARAM_STR);
		$req->execute();
	}
	...
	{	
		...
		$txt_req = $txt_req . " WHERE mrbs_entry.create_by = :nom";
	...
	{	
		...
		{	// création d'un objet Reservation
			...
			$unAreaName = $uneLigne->area_id;
			$unAeraAdminEmail= $uneLigne->room_admin_email;
			
			$uneSalle = new Salle($unId, $unRoomName, $unCapacity, $unAreaName, $unAeraAdminEmail);
			// ajout de la réservation à la collection
			$listeSalles[] = $uneSalle;
			// extrait la ligne suivante
			$uneLigne = $req->fetch(PDO::FETCH_OBJ);
		}
		// libère les ressources du jeu de données
